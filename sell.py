...
def sell(prod_name: str, sell_price: float, sell_date: str, amount: int):
    ...
    if checkSellDate == True:
        ...
        # error message if there are not enough items in inventory to sell
        else:
            console.print(f"Amount exceeds product count in inventory! You can currently sell {len(items_to_be_sold)} of {prod_name}.")
